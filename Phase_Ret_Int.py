...
if rank>0:
	...
	if args.svd:
		...
		GBAR_svd=svd_model(GBAR, nmodes=1)
	...
	f_d_red = np.fft.fftfreq(nt2 + nt2//4, d=times[1]).to(u.mHz)
...
if rank>=1:
	...
	fft_2[:]/=(2*np.pi*f_d.to(u.Hz)[np.newaxis,:].value)**2
	fft_2[:,0]=0
	fft_object_B()
	Smat=np.zeros((nf2 + nf2//4,nt2 + nt2//4,nf2 + nf2//4,nt2 + nt2//4),dtype=complex)
	Smat[:,:,:,:] = fft_1[
	...
	F_mat=np.exp(2j*np.pi*times[np.newaxis,:nt2+nt2//4].value*f_d_red.to(u.Hz)[:,np.newaxis].value)
	D1=np.linalg.inv(F_mat) @ np.diag(2j*np.pi*f_d_red.to(u.Hz).value) @ F_mat
	D_mat = scipy.linalg.block_diag(*(D1 for i in range(nf2+nf2//4)))
	del D1,F_mat
def phase_filter(args):
	...
	if j==0:
		...
		t_stop=t_start + nt2 + nt2//4
	ts = MPI.Wtime()
	M = np.tile(Vmsk[t_start:t_stop],nf2 + nf2//4)
	H = D_mat * M[:,np.newaxis]*M
	N_arr=np.diag(var_interp(np.abs(V_filt[f_start:f_stop,t_start:t_stop]).flatten()))
	L = (Smat @ np.conjugate(H.T)) @ np.linalg.inv( (H @ Smat @ np.conjugate(H.T)) + N_arr)
	filt = np.reshape(
		(L @ (np.angle(V_filt[f_start:f_stop,t_start:t_stop])*Vmsk[t_start:t_stop]).flatten().T).T,
		(nf2 + nf2//4,nt2 + nt2//4))[i*nf2-f_start:(i+1)*nf2-f_start,:]
	print('Rank %s: %s' %(rank,MPI.Wtime()-ts),flush=True)
	if j>0:
		filt-=filt[:,j*nt2-t_start-1][:,np.newaxis]
	return(i,j,filt[:,j*nt2-t_start:(j+1)*nt2-t_start])
...
if not pool.is_master():
	...
	for i,j,arr in results:
		phase_filt[i*nf2:(i+1)*nf2,j*nt2:(j+1)*nt2]=arr.real
	for j in range(bf_t-1):
		phase_filt[:,(j+1)*nt2:(j+2)*nt2]+=phase_filt[:,(j+1)*nt2-1][:,np.newaxis]
	np.save('%s/DB_b0834_phasefilt_int_%s.npy' % (dirname,specs),phase_filt)
	fig, axes = plt.subplots(
